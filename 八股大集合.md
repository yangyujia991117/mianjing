...
<font color="red">在满足第一范式的基础上，要求属性完全依赖于主键，而不能依赖于主键的真子集</font>

“主键”指能够唯一标识一行的一个属性或属性组

“依赖”指：若属性/属性组X确定，那么Y就确定，则Y依赖于X，即X→Y

例子：

有一个订单信息表（<u>订单编号，商品编号</u>，下单时间，商品名称），由于一个订单可能有多种商品，因此主键为（订单编号，商品编号），这样违反了2NF，因为  订单编号→下单时间，商品编号→商品名称，也就是存在属性依赖于主键的真子集。修改方式为拆成三个表，一个是订单表（<u>订单编号</u>，下单时间），一个是商品表（<u>商品编号</u>，商品名称），一个是订单项目表（<u>订单项目ID</u>，订单编号，商品编号）

<font color="red">2NF的目的是让每张表都只能保存一类实体的信息</font>

#### 第三范式(3NF)

<font color="red">在满足第一、第二范式的基础上，要求所有属性都不能依赖于其他非主属性，只能直接依赖于主键</font>
