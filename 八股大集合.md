...
原因：“Integer i=整数;”这句话做了一次自动装箱，把基本数据类型转为包装类型，反编译得知是通过“Integer.valueOf(整数);”实现。<font color="red">Integer中有一个缓存机制，提前创建了[-128,127]之间的Integer对象并保存在缓存中，当调用Integer.valueOf(int)方法进行自动装箱时，如果传进的int范围在[-128,127]之中，则直接返回缓存中对应的Integer对象。</font>因此上面代码的a1和b1指向的是同一个对象
...
<font color="red">属性不能再分</font>

关系型数据库的表一定满足第一范式

#### 第二范式(2NF)

<font color="red">在满足第一范式的基础上，要求没有属性**部分依赖**于候选键，换句话说，每个非主属性都应该**完全依赖**于候选键（或主键），不能只依赖于候选键（或主键）的一部分</font>

“候选键”指能够唯一标识一行的最小属性集合，每张表可以有多个候选键

“主键”是一个特殊的候选键，每张表只能有一个主键

“主属性”指在候选键中出现的属性，"非主属性"指不在任何一个候选键中出现的属性

“依赖”指若属性/属性组X确定，那么Y就确定，则Y依赖于X，即X→Y；“完全依赖”指X→Y时，Y不依赖于X的任何一个真子集；“部份依赖”指X→Y时，Y依赖于X的某一个真子集

例子：

有一个订单信息表（<u>订单编号，商品编号</u>，下单时间，商品名称），由于一个订单可能有多种商品，主键为（订单编号，商品编号），这样违反了2NF，因为  订单编号→下单时间，商品编号→商品名称，也就是存在属性依赖于主键的真子集。修改方式为拆成三个表，一个是订单表（<u>订单编号</u>，下单时间），一个是商品表（<u>商品编号</u>，商品名称），一个是订单项目表（<u>订单项目ID</u>，订单编号，商品编号）

<font color="red">2NF的目的是让每张表都只能保存一类实体的信息</font>

#### 第三范式(3NF)

<font color="red">在满足第一、第二范式的基础上，消除非主属性对候选键的传递依赖。要求所有**非主属性**都不能依赖于其他非主属性，只能直接依赖于候选键（但是主属性可以依赖于非主属性，这是一个缺陷）</font>

例子：

有订单信息表（<u>订单编号</u>，下单人编号，下单人姓名），其中下单人编号→下单人姓名，因此不满足3NF。修改方式为拆成两个表，一个是订单信息表（<u>订单编号</u>，下单人编号），一个是下单人信息表（<u>下单人编号</u>，下单人姓名），其中订单信息表中下单人编号是外键

**补充：BCNF（修正的第三范式）**

<font color="red">在满足3NF的基础上，**主属性和非主属性**都必须完全依赖于候选键，不能依赖于非主属性</font>

对于主属性来说，它要么就不依赖于任何别的属性组，如果它有依赖的属性组，那么该属性组必须是候选码

### 3.关系型数据库和非关系型数据库的比较

1. 数据模型
   - 关系型数据库：表格形式，结构化数据
   - 非关系型数据库：数据模型不固定，可以是半结构化或非结构化的，如键值对（redis）、JSON文档（mongoDB)、图（neo4j)等
2. 查询语言
   - 关系型数据库：使用结构化的SQL语句
   - 非关系型数据库：根据具体的数据库使用自己的查询语言或API
3. 一致性和性能
   - 关系型数据库：强调事务的ACID特性，强调数据的一致性和完整性
   - 非关系型数据库：大部分不支持ACID事务，为了可扩展、高性能牺牲了一部分一致性，性能较高。<font color="grey">在分布式NOSQL场景下，一般不要求强一致性，满足BASE理念即可（基本可用、最终一致性、软状态）</font>
4. 扩展性与架构设计
   - 关系型数据库：垂直扩展（增加硬件资源）、读写分离、分库分表，一般为单点或主从复制架构
   - 非关系型数据库：支持水平扩展（增加节点处理大规模数据），并具有分布式架构
5. 适用场景
   - 关系型数据库：事务性应用、需要复杂查询、数据结构相对稳定的场景
   - 非关系型数据库：大数据、需要高可用性和扩展性的场景，以及数据结构灵活、需要快速迭代的场景。
...
执行limit[m,n]时需要全表搜索，按顺序读取到第m行数据，若m很大，那么速度会很慢

优化思路1：添加适当的主键where条件，例如要执行select * from table limit[1000,10];时新增一个where子句，变为：select * from table where id>=800 limit[1000,10];

优化思路2：使用适当的索引

优化思路3：如果能大概得知目标数据的时间的话，把查询的数据范围限定在对应的日期内
...
- 快照读（普通select，当前读读的都是最新数据，没用MVCC）：
...
1. 如果<font color="red">当前活跃事务最小ID≤事务ID＜下一个要分配的事务ID</font>，表明创建这个read view的时候修改这行的那个事务可能活跃了也可能已提交，这时候<font color="red">看当前活跃事务ID列表中是否有这个事务ID</font>，如果没有的话说明创建这个read view的时候这个事务已经提交了，可见，否则不可见
...
2. **使用索引，并且在建立联合索引时注意索引字段的顺序**
3. **统一字符编码，均用UTF8，防止因为不同字符集进行比较前进行编码转换导致索引失效**
4. **尽量控制单表数据量大小，可以用分库分表等手段控制数据量大小**
5. **经常使用的列放到一个表中，避免更多的表关联操作**
6. **不要存图片等大的二进制数据，存其地址即可**
7. 避免使用TEXT、BLOB等大数据类型，因为当查询这类数据时，在排序等操作时，不嫩使用内存临时表，必须使用磁盘临时表，性能很差；另外给这种类型的字段加索引时只能使用前缀索引（因为索引字段长度有限制）
8. 某些字符串可以转换成数字类型存储（如IP地址）
9. 小数值类型，优先使用TINYINT类型
10. 尽量把所有列定义为NOT NULL，因为索引NULL列需要额外空间保存，且进行比较和计算时要对NULL值做特别处理
11. float和double会丢失精度，在避免丢失数字精度的场景使用<font color="red">decimal(p,d)</font>，表示存储一个最多p个数字，小数位数为d位的数字，例如decimal(6,2)存储范围为-9999.99~9999.99
...
栈和堆位于内存（RAM）中，堆栈中一些常用数据放在缓存中（栈中的常用数据放在一级缓存，堆中的常用数据放在二级缓存），在执行一些计算的时候会用到寄存器，在读取文件、保存文件的时候会用到硬盘，例如通过BufferedReader从标准输入中读取字符串。

### 2.介绍一下操作系统的用户态和内核态

用户态：普通用户程序的执行状态，在用户态下程序只能访问被允许的资源和执行受限制的操作，不能直接访问系统核心资源或执行特权指令。

内核态：操作系统拥有最高权限的状态，内核态运行的进程几乎可以访问计算机的任何资源，包括系统的内存空间、硬件设备等，可以执行特权指令，拥有很高的权限

区分用户态和内核态的原因：

1. 在CPU的所有指令中，有一些指令比较危险，例如内存分配、IO处理等，也被称为“特权指令”。如果所有程序都能使用这些特权指令，会影响系统的正常运行，所以限制只能由内核态执行。
2. 如果只有一个内核态，那么所有程序和进程都必须共享系统资源，会导致系统资源的竞争和冲突，影响系统性能，所有程序和进程都具有相同的特权级别和访问权限也会使得系统的安全性降低。

### 2.操作系统从用户态切换到内核态的几种情况

- 系统调用：用户态进程<u>主动调用系统调用接口</u>，主动切换到内核态，目的是执行一些内核态才能做的操作。例如fork()接口实际上就是执行了一个创建新进程的系统调用
- 中断：外围设备完成一些操作时，会<u>向CPU发出中断信号</u>，这时候CPU会暂停执行下一条即将执行的指令，根据<u>中断向量表</u>找到相应的处理程序后，转而执行中断信号对应的处理程序。如果先前执行的指令是用户态下的程序，那么这个转换的过程自然也就发生了由用户态到内核态的切换。例如磁盘I/O完成时，会发送一条中断信号，系统会切换到硬盘I/O的中断处理程序中执行后续操作。
- 异常：当 CPU 在执行运行在用户态下的程序时，发生了某些事先不可知的异常，例如缺页异常时，会根据中断向量表找到相应的异常处理程序，执行内核的中断处理程序。
...
每个类或模块应该只有一个职责，也就是有且仅有一个引起它变化的原因

### 2.开闭原则

软件应对扩展开放，对修改关闭。即软件实体应该通过扩展来增加功能，而不是通过修改已有的代码。

### 3.里氏替换原则

任何父类出现的地方，子类都可以将其替换掉

### 4.接口隔离原则

应将大接口分割成较小的接口，客户端不应该依赖它不使用的接口

### 5.依赖倒置原则

上层模块不应该依赖下层模块，两者都应该依赖于抽象，也就是面向接口编程

### 6.合成复用原则

尽量使用组合/聚合，而不是继承来实现代码复用

### 7.迪米特法则（最小知识原则）

一个对象应该对其他对象保持最少的了解。比如一个对象不应该直接调用另一个对象的内部方法，而应该通过对象的公共接口来进行通信。
...
如上，海尔具体工厂和TCL具体工厂都继承于EFactory这个抽象工厂类，这两个具体工厂可以生产对应产品族的产品，例如海尔具体工厂可以生产海尔电视和海尔空调，TCL具体工厂可以生产TCL电视和TCL空调，客户端代码如下：
...
1. 限流控制：限制每个用户或IP地址在单位时间可以提交的请求数量
2. 验证码验证：在关键操作（如提交订单）前要求用户进行验证码验证，防止自动化脚本或机器人攻击
3. IP黑名单与白名单：维护IP黑名单与白名单，阻止已知或者可疑的恶意IP发出请求
4. 限购策略：限制每个商品每个用户可以购买的最大数目
...
1. 先对大文件进行排序，使得重复的数聚集在一次，排序算法可以用前面的<font color="red">分治+外存保存中间结果+多路归并+败者树</font>的方法
2. 遍历排序好的结果，遍历的过程中就知道了每个数的出现次数，用容量为k的最小堆给每个数的出现次数排序，前k个出现次数直接添加进最小堆中，后面的就和堆顶比较
...
chatGPT是在GPT(生成式预训练变换器模型Gennerative pre-training transformer)的基础上，针对对话场景特别优化过的，它可以根据上下文自动生成和人类一样的文本对话。



chatGPT涉及的关键技术：

1. Transformer模型：chatGPT基于Transformer架构（一种深度学习模型），该模型利用**自注意力机制**来捕捉输入序列中不同位置之间的依赖关系，从而处理长文本序列。
2. 预训练：大模型通过学习大量的文本数据来获取丰富的语言知识和语境理解能力。这使得chatGPT能够理解和生成自然语言文本。chatGPT还制定了reward（奖励）算法根据生成的结果进行调整
3. 微调：在预训练之后，可以通过微调来使模型更适应特定的任务或应用场景，可以使用特定的数据集和标签来进一步训练模型。

使用chatGPT完成一次问答的过程：

1. 输入一段文字
2. chatGPT将输入文本转换为数字向量。详细步骤如下：
   - 分词：首先输入的文本被分割成单词的序列，这些单词被称为标记(tokens)
   - 嵌入：接着，每个标记会被映射为一个高维向量空间中的向量，这些向量称为嵌入(embeddings)，可以用来捕捉单词之间的语义关系
   - 位置编码：为输入的序列添加位置编码，以表示单词在序列中的位置信息
   - 输入向量拼接：将嵌入的标记向量和位置编码相加，也就是语义关系和位置信息相加，得到最终的输入向量表示
3. chatGPT用预训练好的Transformer模型对输入文本进行推理，生成与问题相关的回答。详细步骤如下：
   - 自注意力计算：通过自注意力机制，模型根据输入序列中的所有单词之间的依赖关系来计算每个单词的表示。
   - 多头注意力：为了提高模型对不同特征和语义的表达能力，通常使用多个自注意力头来进行计算。
   - 前向传播：经过自注意力计算后，每个单词的表示将通过前向传播网络进行处理，以进一步提取特征和语义信息。
   - 输出生成：最后，模型根据计算得到的表示，通过softmax函数生成概率分布，从而生成最终的输出文本。
